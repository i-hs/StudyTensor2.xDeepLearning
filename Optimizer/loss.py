...
class SimpleNetwork:
    def __init__(self):
        ...
        # 가중치 행렬W(2x3)의 초기값 랜덤 추출
        # randn(n: normal distribution을 만족하는 숫자 랜덤 추출): shape (row, column)

    def predict(self, x):
        z = x.dot(self.W)  # 행렬곱 X * W
        y = softmax(z)  # softmax 활성화 함수 적용
        # print('predict:', y)
        return y
    ...
    def loss(self, x, y_true):
        ...
        """
        y_pred = self.predict(x)  # X * W 값 = y_pred
        ce = cross_entropy(y_pred, y_true)  # y_pred와 y_true간의 cross_entropy 값 계산 = ce
        # print('ce:', ce)
        return ce

    def gradient(self, x, t):
        """ x: 입력, t: 출력 실제 값(정답 레이블)"""
        fn = lambda W: self.loss(x, t)  # training data x에 대한 손실함수를 계산하여 편미분 // lambda 변수 : 함수
        # print('fn:', fn)
        return partial_gradient(fn, self.W)  # 손실함수 계산값, 현재 비중W -> 편미분값
